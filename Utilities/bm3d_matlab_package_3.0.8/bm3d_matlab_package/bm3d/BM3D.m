...
for psd_num = 1:size(PSD65_full, 3)
    ...
    mm = mean(features, 2);
    centered_features = bsxfun(@minus, features, mm); %features - mm;
    corr_matx = centered_features * centered_features';
    corr_matx = corr_matx / 500;
    
    centered_pcax = pcaxa' - mm; % bsxfun(@minus, pcaxa', mm); %
    
    [u, s, ~] = svd(corr_matx);
    centered_features = u * centered_features;
    centered_pcax = u * centered_pcax;
    centered_features = bsxfun(@times, centered_features, sqrt(diag(s))); %centered_features .* sqrt(diag(s));
    centered_pcax = centered_pcax .* sqrt(diag(s)); % centered_pcax = bsxfun(@times, centered_pcax, sqrt(diag(s))); %
    %diff_pcax = sqrt(sum(abs(centered_features - centered_pcax).^2, 1));
    diff_pcax = sqrt(sum(abs(bsxfun(@minus, centered_features, centered_pcax)).^2, 1));
    ...
    param_idxs = sum(bsxfun(@times, diff_inv, maxes(diff_indices, :)') , 2);
    lambdas = 2.5:0.1:5;
