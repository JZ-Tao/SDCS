function [Out,W_hyper,H_hyper,W_multi,H_multi] = CNMF_fusion_mod(HSI,MSI,MTF_info, n_sub, mask)
%--------------------------------------------------------------------------
...
if ~exist('MTF_info', 'var')
	down_func = @(x,w) imresize(x,1/w);
else
	down_func = @(x,w) conv_downsample_MTF(x,w,MTF_info);
end
% image size
[rows1,cols1,bands1] = size(MSI);
[rows2,cols2,bands2] = size(HSI);
w = rows1/rows2;
LR_MS = down_func(MSI,w);
% estimation of R
if masking == 1
    [R,error] = estR_new(HSI,LR_MS,mask);
    for b = 1:bands1
...
switch masking
    case 0
        Out = CNMF_init_mod(rows1,cols1,w,M,HSI,MSI,sum2one,I1,th_h,th_m,R,init_mode,MTF_info);
    case 1
        Out = CNMF_init_mod(rows1,cols1,w,M,HSI,MSI,sum2one,I1,th_h,th_m,R,init_mode,MTF_info,mask);
end
HSI = Out.hyper;
MSI = Out.multi;
W_hyper = Out.W_hyper;
...
for i = 1:I2
    switch masking
        case 0
            Out = CNMF_ite_mod(rows1,cols1,w,M,HSI,MSI,W_hyper,H_hyper,W_multi,H_multi,I1,th_h,th_m,I2,i,R,MTF_info);
        case 1
            Out = CNMF_ite_mod(rows1,cols1,w,M,HSI,MSI,W_hyper,H_hyper,W_multi,H_multi,I1,th_h,th_m,I2,i,R,MTF_info,mask);
    end
