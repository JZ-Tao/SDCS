function Out = CNMF_ite_mod(xdata,ydata,w,M,hyper,multi,W_hyper,H_hyper,W_multi,H_multi,I_in,delta_h,delta_m,I_out,i_out,srf,MTF_info,mask)
%--------------------------------------------------------------------------
...
switch masking
    case 0
        H_hyper = reshape(down_func(reshape(H_multi',xdata,ydata,M),w),[],M)';
    case 1
        mask2 = imresize(mask,w,'nearest');
        for q = 1:M
            tmp = zeros(xdata,ydata);
            tmp(mask2) = H_multi(q,:);
            tmp = reshape(down_func(tmp,w),1,[]);
