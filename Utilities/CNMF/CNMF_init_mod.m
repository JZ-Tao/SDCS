function Out = CNMF_init_mod(xdata,ydata,w,M,hyper,multi,delta,I_in,delta_h,delta_m,srf,init_mode,MTF_info,mask)
%--------------------------------------------------------------------------
...
if ~exist('MTF_info', 'var')
	up_func = @(x,w) imresize(x,w);
else
	up_func = @(x,w) interpWrapper(x,w,MTF_info);
end
...
switch masking
    case 0
        ...
        for i = 1:M
            tmp = up_func(reshape(H_hyper(i,:),hx,hy),w);
